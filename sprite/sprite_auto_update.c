...
loff_t fat_fs_read(const char *filename, void *buf, int offset, int len)
{
	...
	fs_read(filename, (ulong)buf, offset, len, &len_read);
			 ...
			 char *const argv[])
{
	...
	if (simple_strtoul(argv[1], NULL, 16) == 1) {
		...
		file_size = 0;
		file_offset = 0;
		fs_read("scripts/auto_update.txt", (ulong)file_p,
				file_offset, file_size, &len_read);
		...
		for (i = 0; i < arg_max; i++) {
			...
			if (file_size < MAX_FILE_SIZE) {
				...
				if (fs_set_blk_dev(interface, "0", FS_TYPE_FAT))
					return 1;
				file_offset = 0;
				fs_read(commands[i].argv[2], (ulong)file_buff,
						file_offset, file_size, &len_read);
				...
				while (file_size) {
					if (file_size >= MAX_FILE_SIZE) {
						if (fs_set_blk_dev(interface, "0", FS_TYPE_FAT))
							return 1;
						fs_read(commands[i].argv[2],
							(ulong)file_p, file_offset,
							MAX_FILE_SIZE, &len_read);
						mdelay(10);
