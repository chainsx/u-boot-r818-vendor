 ...
 * nand-partitions for sunxi rawnand partition
 *
 * Copyright (C) 2019 Allwinner.
 * Autor: cuizhikui<cuizhikui@allwinnertech.com>
 * Date : 2021.09.18
 */
...
void print_bad_blocks(struct _nand_super_block *badblocks, int len)
{
	...
	for (i = 0; i < len; i++) {
		if (badblocks[i].Chip_NO != 0xffff && badblocks[i].Block_NO != 0xffff)
			printf("%d-%d ", badblocks[i].Chip_NO, badblocks[i].Block_NO);
	}
	printf("\n");
}
...
int nand_scan_phy_partitions_bad_blocks(struct _nand_info *nand_info)
{
	...
	unsigned int multi_plane = nand_get_twoplane_flag();
	unsigned int multi_plane_blkoffset = nand_get_chip_multi_plane_block_offset();

	bool had_detect_badblock = false;

	if (ubootsta.nand_erase_state.phy_erase == true)
		had_detect_badblock = true;

	if (had_detect_badblock == true && nand_info->FirstBuild == 1) {
		for (die = nand_info->no_used_block_addr.Chip_NO;
			...
			for (block = nand_info->no_used_block_addr.Block_NO;
				...
				if (multi_plane) {
					if (phy_block_is_bad_detect_by_erasechip(die, (block << 1)) ||
							phy_block_is_bad_detect_by_erasechip(die, (block << 1) + multi_plane_blkoffset)) {
					...
					if ( phy_block_is_bad_detect_by_erasechip(die, block)) {
						nand_info->factory_bad_block[factory_bad_blocks].Chip_NO = die;
						nand_info->factory_bad_block[factory_bad_blocks].Block_NO = block;
						factory_bad_blocks++;
						if (factory_bad_blocks == FACTORY_BAD_BLOCK_SIZE / sizeof(struct _nand_super_block)) {
		...
		bad_blocks = factory_bad_blocks;
		if (bad_blocks != 0) {
			printf("factory bad block:\n");
			print_bad_blocks(nand_info->factory_bad_block, bad_blocks);
		}
	} else if (nand_info->FirstBuild == 1) {
		for (die = nand_info->no_used_block_addr.Chip_NO;
				die < nand_info->ChipNum; die++) {
			for (block = nand_info->no_used_block_addr.Block_NO;
					block < nand_info->BlkPerChip; block++) {

				if (ubootsta.nand_bootinfo_state.state == BOOT_INFO_OK) {
...
int nand_build_boot_info_f(struct _nand_info *nand_info, int state)
{
	...
	if (nand_info->bootinfo_is_old == BOOTINFO_OLD &&
		...
		nand_parts2_to_parts3(&nand_info->boot->parts, &nand_info->boot->mbr.data, &nand_info->boot->partition,
				nand_info->boot->partition.data[0].size);
	} else {
...
int nand_build_boot_info_r(struct _nand_info *nand_info, int state)
{
	...
	struct _boot_info *boot_info = NULL;
...
int nand_build_phy_partitions(struct _nand_phy_partition **partition_head, int state)
{
	...
	phy_partition = (struct _nand_phy_partition *)malloc_align(
			sizeof(struct _nand_phy_partition), 64);
...
struct _nand_partition *build_nand_partition(struct _nand_phy_partition *phy_partition)
{
	...
	if (phy_partition == NULL) {
		pr_err("%s phy_partition is null\n", __func__);
		return NULL;
	}
...
int nand_get_parts(char *buffer, uint len)
{
	int n = 0;

	sunxi_mbr_t *mbr = (sunxi_mbr_t *)buffer;

	nand_parts.nc = mbr->PartCount + 1;

	/*mbr part fix to 16K*/
	nand_parts.parts[0].from = 0;
	nand_parts.parts[0].size = 0x8000;
	nand_parts.size = nand_parts.parts[0].size;

	/*
	 *pr_info("nand part@%u: from@%x size@%x\n", 0,
	 *                        nand_parts.parts[0].from, nand_parts.parts[0].size);
	 */

	for (n = 1; n < nand_parts.nc; n++) {
		nand_parts.parts[n].from = nand_parts.parts[n - 1].from +
			nand_parts.parts[n - 1].size;
		nand_parts.parts[n].size = mbr->array[n - 1].lenlo;
		nand_parts.size += nand_parts.parts[n].size;
		/*
		 *pr_info("nand part@%u: from@%x size@%x\n", n,
		 *                nand_parts.parts[n].from, nand_parts.parts[n].size);
		 */
	}
	return 0;
}
...
int nand_info_init(struct _nand_info *nand_info, int state)
{
	...
	nand_info->FirstBuild = (state == UBOOT_IN_PRODUCT) ? 1 : 0;
	...
	nand_info->temp_page_buf = malloc_align(bytes_per_page, 64);
	...
	nand_info->factory_bad_block = malloc_align(FACTORY_BAD_BLOCK_SIZE, 64);
	...
	nand_info->new_bad_block = malloc_align(PHY_PARTITION_BAD_BLOCK_SIZE, 64);
	...
	if ((state == UBOOT_IN_PRODUCT) && ((udisk_size = nand_calc_udisk_capacity(nand_info)) == NOSPACE))
		goto out;
	else if (state == UBOOT_IN_PRODUCT) {
...
out_nomem:
	if (nand_info->temp_page_buf)
		free_align(nand_info->temp_page_buf);
	if (nand_info->factory_bad_block)
		free_align(nand_info->factory_bad_block);
	if (nand_info->new_bad_block)
		free_align(nand_info->new_bad_block);
	if (nand_info->mbr_data)
		free_align(nand_info->mbr_data);
	return ret;
}
